...
if command -v nwa &> /dev/null; then
  nwa add -l apache -c "$AUTHOR" "src/**/*.{ts,tsx}" "scripts/**/*.{sh,ts}"
elif command -v docker &> /dev/null; then
  docker run --rm -v "${PWD}:/src" ghcr.io/b1nary-gr0up/nwa:main add -l apache -c "$AUTHOR" "src/**/*.{ts,tsx}" "scripts/**/*.{sh,ts}"
else
